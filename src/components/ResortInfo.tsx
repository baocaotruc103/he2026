  ...
  return (
    <div className="space-y-6" id="resort-info-page">
      ...
      <div className="flex items-center gap-3 mb-4 bg-white border border-slate-200/60 p-4.5 rounded-2xl shadow-sm">
        ...
        <div>
          <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Ruby Star Beach Quynh Resort</h2>
          <p className="text-sm text-slate-500 font-medium">Thông tin điểm đến nghỉ dưỡng hè chi tiết từ Ban tổ chức</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
          <div className="space-y-1">
            <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Địa chỉ resort</p>
            <h3 className="text-lg font-extrabold text-slate-800 flex items-center gap-1.5">
              <MapPin className="w-4 h-4 text-rose-500 shrink-0" />
              Thôn 3, Quỳnh Phú, Nghệ An
            </h3>
          </div>
          <div className="bg-emerald-50 text-[#059669] font-extrabold text-sm px-3.5 py-1.5 rounded-xl border border-emerald-100 flex items-center gap-1.5 self-start sm:self-auto">
          ...
          {scheduleTimes.map((item, idx) => (
            <div key={idx} className="bg-slate-50/50 border border-slate-100 p-4 rounded-xl flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-black text-slate-800 mt-0.5">{item.value}</p>
                <p className="text-[13px] text-slate-500 font-medium mt-0.5">{item.note}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4">
        <h4 className="font-extrabold text-base text-slate-800 flex items-center gap-2 border-b border-slate-100 pb-3">
            ...
            <div key={idx} className="bg-slate-50/40 border border-slate-100 p-4 rounded-xl space-y-2 flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-center border-b border-slate-100 pb-1.5 mb-2">
                  <span className="font-black text-slate-800 text-sm uppercase">{meal.type}</span>
                  <span className="bg-emerald-50 text-[#059669] text-xs font-extrabold px-2 py-0.5 rounded-lg border border-emerald-100">
                    {meal.time}
                  </span>
                </div>
                <p className="text-[13px] font-bold text-[#059669] flex items-center gap-1">
                  <MapPin className="w-3 h-3" /> {meal.place}
                </p>
                <p className="text-[13px] text-slate-500 leading-relaxed mt-1.5 font-medium">{meal.desc}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="space-y-3.5">
            {freeServices.map((service, idx) => (
              <div key={idx} className="flex gap-3 items-start p-2.5 bg-slate-50/50 border border-slate-100/60 rounded-xl">
                ...
                <div>
                  <h5 className="text-sm font-black text-slate-800">{service.title}</h5>
                  <p className="text-[13px] text-slate-500 font-medium mt-0.5 leading-relaxed">{service.desc}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            {roomRegulations.map((reg, idx) => (
              ...
              >
                <div className="flex items-center gap-1.5 border-b border-slate-100/30 pb-1 mb-1.5">
                  ...
                  <span className="text-[13px] font-black uppercase text-slate-800 tracking-wide">{reg.title}</span>
                </div>
                <p className="text-[13px] text-slate-600 font-medium">{reg.content}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-slate-900 text-white rounded-2xl p-6 shadow-md relative overflow-hidden space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
          <div>
            <h4 className="font-black text-base text-white tracking-wide uppercase">LIÊN HỆ & TRỢ GIÚP ĐOÀN</h4>
            <p className="text-[13px] text-slate-400 font-medium mt-0.5">Mọi vướng mắc xin vui lòng thông báo theo các kênh chính thức sau</p>
          </div>
          <a
            href="tel:0833464646"
            className="inline-flex items-center gap-2 bg-[#059669] hover:bg-[#047857] text-white font-black text-sm px-4 py-2 rounded-xl transition-all shadow-md shadow-emerald-950/20 self-start sm:self-auto cursor-pointer"
          >
        ...
        <div className="flex gap-3 text-sm text-slate-300 bg-slate-850 p-4 rounded-xl border border-slate-800/80 leading-relaxed font-medium">
