  ...
  return (
    <div className="space-y-6" id="driver-info-page">
      ...
      <div className="flex items-center gap-3 mb-4 bg-white border border-slate-200/60 p-4.5 rounded-2xl shadow-sm">
        ...
        <div>
          <h2 className="text-lg font-bold text-slate-800 uppercase tracking-tight">Thông tin lái xe</h2>
        </div>
      </div>
        ...
        {drivers.map((driver) => (
          <div key={driver.id} className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm hover:shadow-md transition-all space-y-4">
            ...
            <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
              <span className="text-base font-black text-[#059669] uppercase tracking-tight">{driver.busNumber}</span>
              <span className="bg-emerald-50 text-[#059669] text-[10px] font-black uppercase px-2.5 py-1 rounded-xl border border-emerald-100">
                Đang hoạt động
              </span>
            </div>
            ...
            <div className="space-y-3.5 text-xs text-slate-700">
              ...
              <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm space-y-2.5">
                <div className="flex justify-between items-center">
                  <span className="text-slate-400 font-extrabold uppercase tracking-wider text-[10px]">Biển số:</span>
                  <span className="font-black text-slate-800 text-sm">{driver.plate}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-400 font-extrabold uppercase tracking-wider text-[10px]">Tên lái xe:</span>
                  <span className="font-extrabold text-slate-800">{driver.driverName}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-400 font-extrabold uppercase tracking-wider text-[10px]">SĐT:</span>
                  <div className="flex items-center gap-2">
                    <span className="font-extrabold text-[#059669] text-sm">{driver.driverPhone}</span>
                    <a
                      href={`tel:${driver.driverPhone}`}
                      className="inline-flex items-center gap-1 bg-emerald-50 hover:bg-emerald-100 text-[#059669] font-black text-[10px] px-2.5 py-1 rounded-lg border border-emerald-200/60 transition-colors cursor-pointer"
                    >
              ...
              <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm space-y-2.5">
                <div className="flex justify-between items-center">
                  <span className="text-slate-400 font-extrabold uppercase tracking-wider text-[10px]">Phụ trách xe:</span>
                  <span className="font-extrabold text-slate-800">{driver.leaderName}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-400 font-extrabold uppercase tracking-wider text-[10px]">SĐT:</span>
                  <div className="flex items-center gap-2">
                    <span className="font-extrabold text-slate-800 text-sm">{driver.leaderPhone}</span>
                    <a
                      href={`tel:${driver.leaderPhone}`}
                      className="inline-flex items-center gap-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-black text-[10px] px-2.5 py-1 rounded-lg border border-slate-200 transition-colors cursor-pointer"
                    >
                      <Phone className="w-3 h-3" />
                      Gọi
                    </a>
      ...
      <div className="bg-emerald-50/50 border border-emerald-100/40 rounded-2xl p-4 flex gap-3 text-xs leading-relaxed text-slate-600">
